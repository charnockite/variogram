...
//testing
//console.log(calculateVariogram(testData, 50, 5, 0, 5))
...
function calculateVariogram(data, lagDistance, numberOfLags, azimuth, tolerance){
  ...
  let sliceValue = data.length / 2;
  if (data.length % 2 != 0){
    sliceValue = Math.floor(data.length /2);
  }
  let pointResults = data.slice(0,sliceValue).map(x => getDistancesForPoint(x));
  //flatten list
  let flatList = [].concat.apply([],pointResults);
  //for each lag:
  output = []
  for (let i = 1; i <= numberOfLags; i++){
    //filter pointResults according to direction
    let distanceFiltered = flatList.filter(element => element[0] > (i-1) * lagDistance && element[0] <= i * lagDistance);
